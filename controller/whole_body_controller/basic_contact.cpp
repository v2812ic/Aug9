...
SurfaceContact::SurfaceContact(PinocchioRobotSystem *robot,
                               const int target_link_idx, const double mu,
                               const double x, const double y, const double off)
    : Contact(robot, 6, target_link_idx, mu), x_(x), y_(y), off_(off) {
  util::PrettyConstructor(3, "SurfaceContact");

  int neq = 16 + 2;
  cone_constraint_matrix_ = Eigen::MatrixXd::Zero(neq, dim_);
  cone_constraint_vector_ = Eigen::VectorXd::Zero(neq);
  cone_constraint_transform_ = Eigen::MatrixXd::Identity(dim_, dim_);
  rot_w_l_ = Eigen::MatrixXd::Zero(dim_, dim_);
}
...
// Aquí vamos a meter una transformación a la cone_constraint_matrix para offsetear lo del pie
...
void SurfaceContact::UpdateConeConstraint() {
  ...
  if (dim_ == 6){
    ...
    cone_constraint_transform_(2, 4) = -off_;
    cone_constraint_transform_(1, 5) = off_;
    
    cone_constraint_matrix_ = cone_constraint_matrix_ * cone_constraint_transform_;
  }
...
void SurfaceContact::SetParameters(const YAML::Node &node) {
  try {
    ...
    util::ReadParameter(node, "foot_offset", off_);
