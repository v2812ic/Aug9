...
G1CoMZTask::G1CoMZTask(PinocchioRobotSystem *robot) : Task(robot, 1) {

  util::PrettyConstructor(3, "G1CoMZTask");
  sp_ = G1StateProvider::GetStateProvider();
}

void G1CoMZTask::UpdateOpCommand(const Eigen::Matrix3d &world_R_local) {
  if (com_height_ == com_height::kCoM) {
    ...
    vel_ << robot_->GetLinkSpatialVel(g1_link::torso_com_link)[5];
  }
...
void G1CoMZTask::UpdateJacobian() {
  if (com_height_ == com_height::kCoM) {
    jacobian_ = robot_->GetComLinJacobian().bottomRows<1>();
  } else if (com_height_ == com_height::kBase) {
    jacobian_ =
        robot_->GetLinkJacobian(g1_link::torso_com_link).bottomRows<1>();
  }
}

void G1CoMZTask::UpdateJacobianDotQdot() {
  if (com_height_ == com_height::kCoM) {
    jacobian_dot_q_dot_ << robot_->GetComLinJacobianDotQdot()[2];
  } else if (com_height_ == com_height::kBase) {
    jacobian_dot_q_dot_ << robot_->GetLinkJacobianDotQdot(
        g1_link::torso_com_link)[5];
  }
}

void G1CoMZTask::SetParameters(const YAML::Node &node,
