...
MoveFootForward::MoveFootForward(const StateId state_id,
                                           PinocchioRobotSystem *robot,
                                           G1ControlArchitecture *ctrl_arch)
    : StateMachine(state_id, robot), ctrl_arch_(ctrl_arch) {
  util::PrettyConstructor(2, "MovingFootForward");
  nominal_lfoot_iso_.setIdentity();
  nominal_rfoot_iso_.setIdentity();
  sp_ = G1StateProvider::GetStateProvider();
}

void MoveFootForward::FirstVisit() {
  std::cout << "g1_states: Moving Foot Forward" << std::endl;
  state_machine_start_time_ = sp_->current_time_;
  
  if (first_time_){
    end_time_ = state_machine_start_time_ + swing_semitime_;
  }
  else end_time_ = state_machine_start_time_ + 2*swing_semitime_;
  ...
  if (swing_is_left_) {
    ...
    final_iso.translation() += foot_init_pos_swing_offset_;

    if (not first_time_) final_iso.translation() -= foot_end_pos_swing_offset_;
...
bool MoveFootForward::EndOfState() { 
  return (sp_->current_time_ >= end_time_ + wait_time_);
}

void MoveFootForward::LastVisit() {
  // no haga nada y quito el flag de first_time
  first_time_ = false;
  state_machine_time_ = 0.;
}

StateId MoveFootForward::GetNextState() {
  if (swing_foot_)
    return g1_states::MoveFootBackward;
}

void MoveFootForward::SetParameters(const YAML::Node &node) {
  try {
