...
void G1Controller::SetExternalTorque(const Eigen::VectorXd& tau_ext)
{
    if (ihwbc_) {                      // cuando el WBC es IHWBC
        ihwbc_->SetExternalTorque(tau_ext);
    }
    if (wbic_) {                       // cuando usas WBIC
        wbic_->SetExternalTorque(tau_ext);
    }
    for (auto& task : tci_container_->task_map_){
      task.second->SetExternalTorque(tau_ext);
    }
  }
void G1Controller::SetExternalForce(const Eigen::MatrixXd& f_ext)
{
    if (ihwbc_) {                      // cuando el WBC es IHWBC
        ihwbc_->SetExternalForce(f_ext);
    }
    if (wbic_) {                       // cuando usas WBIC
